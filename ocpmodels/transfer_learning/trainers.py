...
class GAPTrainer(BaseTrainer):
    def __init__(
        ...
        self.predictions_dir = self.path_run_dir / "predictions" / self.timestamp_id
        self.predictions_dir.mkdir(parents=True, exist_ok=True)
        self.aux_dir = self.path_run_dir / "aux" / self.timestamp_id
    ...
    def train(self):
        ...
        self.gap_name = self.config["model"].pop("name")
        self.gap_kw_string = self._build_cmd_string(self.gap_kwargs)
        self.gap_kw_string = f"{self.gap_name}=" + "{" + self.gap_kw_stringgap_string + "}"

        self.other_kwargs = self.config["model"]["other_params"]
        self.other_kw_string = self.build_cmd_string(self.other_kwargs)
    ...
    def predict(
        ...
        pred = ase.io.read(self.aux_dir / f"{split}_predictions.xyz", index=b":")
